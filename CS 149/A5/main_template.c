...
{
	p = realloc(p,t);
  	printf("File %s, line %d, %s reallocated new memory segment at address %p to size %d.\n", file, line, PRINT_TRACE(), p, t);
	return p;
}
...
void* MALLOC(int t,char* file,int line)
{
	void* p;
	p = malloc(t);
  	printf("File %s, line %d, %s allocated new memory segment at address %p to size %d.\n", file, line, PRINT_TRACE(), p, t);
	return p;
}
