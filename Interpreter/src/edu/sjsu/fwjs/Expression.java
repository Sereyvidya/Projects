...
class BinOpExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
        Value aVal = e1.evaluate(env);
        Value bVal = e2.evaluate(env);
        if (op == Op.EQ) {
            return new BoolVal(aVal.equals(bVal));
        }
        int a = ((IntVal) aVal).toInt();
        int b = ((IntVal) bVal).toInt();
        switch (op) {
            case ADD:
