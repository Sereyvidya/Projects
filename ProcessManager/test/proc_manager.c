...
// Constant values
const int MAX_NUMBER_OF_COMMAND = 100;
const int MAX_LENGTH_OF_COMMAND = 101; 
const int MAX_EXECVP_ARGS_COUNT = 5;
const int FILE_NAME_LENGTH = 20;

/**
 * This function takes in a text file stream and stores each command line.
 * Assumptions: Command line is at most 100 characters, and at most 100 commands.
 * Input parameters: Input file, array for storing list of commands, count of commands.
 * Returns: void.
 */
void storeCommands(FILE *file,  char commandList[][MAX_LENGTH_OF_COMMAND], int *count) {
    // Array for holding one command
    char command[MAX_LENGTH_OF_COMMAND]; 

    // Iterating through STDIN, and storing each command line
    while (fgets(command, MAX_LENGTH_OF_COMMAND, file) != NULL) {
        ...
        // Stores command in list
        strcpy(commandList[*count], command);

        // Increment count of commands
        (*count)++; 
    }
}
...
int main(int argc, char *argv[]) {
    // Array of all commands and its current count
    char commandList[MAX_NUMBER_OF_COMMAND][MAX_LENGTH_OF_COMMAND]; 
    int count = 0; 
    ...
    storeCommands(stdin, commandList, &count);
    ...
    for (int i = 0; i < count; i++) {
        ...
        separateCommand(args, commandList[i]);
