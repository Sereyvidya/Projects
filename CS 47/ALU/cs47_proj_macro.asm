...
#<------------------ MACRO DEFINITIONS ---------------------->#
	...
	.macro insert_to_nth_bit($regD, $regS, $regT)
		...
		add	$s7, $zero, $regT	# Set $s6 to insert value
		sllv	$s7, $s7, $regS		# Shift $s6 to the left by n
	...
	.macro check_negative($regD, $regS)
		...
		extract_nth_bit($s5, $regD, $s4)
		
		beq 	$s5, 0, check_negative_return
		move	$s3, $regD
		twos_complement($s3)		
		move	$regD, $s3
	...
	.macro twos_complement_64bit($regD, $regS, $regT)
		...
		#move	$s7, $regT		# $regT always contains 1 so move it to $s7
